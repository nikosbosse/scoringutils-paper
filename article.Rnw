...
@
\caption{\label{tab:forecast-types} Ssummary of the different forceast types and forecast formats}
\end{table}
...
df <- data.table::data.table(true_value = rep(true_values, each = n_samples), 
                             ...
                             model = rep(c("Normal(0,1)", "Normal(0.5, 1)", 
                                       ...
                                       "Normal(0.2, 0.7)"), each = n_truth * n_samples))
...
\begin{figure}[h]
\centering
<<score-table, echo=TRUE, fig=TRUE, height=7.2, width=14, eval = TRUE>>=
...
\begin{figure}[h!]
\centering
<<pairwise-plot-ratio, echo=TRUE, fig=TRUE, height=7.2, width=14, eval = TRUE>>=
...
<<quantile-coverage, echo=TRUE, fig=TRUE, height=5.2, width=14, out.width = '0.4\\linewidth', fig.show='hold', eval = TRUE>>=
cov_scores <- eval_forecasts(combined, 
                             summarise_by = c("model", 
                                              "range", "quantile"))
scoringutils::quantile_coverage(cov_scores)
