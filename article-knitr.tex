...
\includegraphics[width=0.98\textwidth]{/mnt/data/GoogleDrive/Uni/PhD/scoringutils-paper/plots/calibration-diagnostic-examples} 
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{locations} \hlkwb{<-} \hlstr{"c(\textbackslash{}"Scotland\textbackslash{}", \textbackslash{}"Wales\textbackslash{}", \textbackslash{}"Northern Ireland\textbackslash{}", \textbackslash{}"England\textbackslash{}")"}
\hlkwd{plot_predictions}\hlstd{(}\hlkwc{truth} \hlstd{= covid_uk_data,} \hlkwc{forecasts} \hlstd{= uk_forecasts,} \hlkwc{filter_both} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwd{paste}\hlstd{(}\hlstr{"geography %in%"}\hlstd{,}
    \hlstd{locations,} \hlkwc{collapse} \hlstd{=} \hlstr{""}\hlstd{),} \hlstr{"value_type == \textbackslash{}"death_inc_line\textbackslash{}""}\hlstd{),} \hlkwc{filter_truth} \hlstd{=} \hlkwd{list}\hlstd{(}\hlstr{"value_date <= \textbackslash{}"2020-06-22\textbackslash{}""}\hlstd{,}
    \hlstr{"value_date > \textbackslash{}"2020-06-01\textbackslash{}""}\hlstd{),} \hlkwc{filter_forecasts} \hlstd{=} \hlkwd{list}\hlstd{(}\hlstr{"model == \textbackslash{}"SIRCOVID\textbackslash{}""}\hlstd{,}
    \hlstr{"creation_date == \textbackslash{}"2020-06-22\textbackslash{}""}\hlstd{),} \hlkwc{x} \hlstd{=} \hlstr{"value_date"}\hlstd{,} \hlkwc{facet_formula} \hlstd{=} \hlopt{~}\hlstd{geography)} \hlopt{+}
    \hlstd{ggplot2}\hlopt{::}\hlkwd{theme}\hlstd{(}\hlkwc{legend.position} \hlstd{=} \hlstr{"bottom"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{wis_components}\hlstd{(scores,} \hlkwc{facet_formula} \hlstd{=} \hlopt{~}\hlstd{value_desc,} \hlkwc{scales} \hlstd{=} \hlstr{"free_x"}\hlstd{,}
    \hlkwc{relative_contributions} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{x_text_angle} \hlstd{=} \hlnum{0}\hlstd{)} \hlopt{+} \hlstd{ggplot2}\hlopt{::}\hlkwd{coord_flip}\hlstd{()} \hlopt{+}
    \hlstd{ggplot2}\hlopt{::}\hlkwd{theme}\hlstd{(}\hlkwc{legend.position} \hlstd{=} \hlstr{"bottom"}\hlstd{)}
\end{alltt}
\end{kframe}
\includegraphics[width=0.98\textwidth]{plots/plot-WIS-components-1} 
\end{knitrout}
